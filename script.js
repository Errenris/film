...
async function initApp() {
    ...
    // Tampilkan Skeleton dulu sebelum data masuk
    ...
    fetchAndRender('discover/movie?with_genres=28', 'row6');                        
}

// =========================================================
// FITUR BARU: FILTER CANGGIH & SURPRISE ME
// =========================================================
...
function applyFilters() {
    ...
    // Sembunyikan panel setelah apply
    document.getElementById('filterPanel').classList.add('hidden');
    
    const label = `Filter: ${year ? year : 'Semua Tahun'} | Rating ${rating ? rating+'+' : 'Semua'}`;
    loadCategory(path, label);
}

async function surpriseMe() {
    // Acak halaman 1-10 dari film populer
    ...
    try {
        ...
        if(data.results && data.results.length > 0) {
            ...
            playMovie(randomMovie.id, randomMovie.title || randomMovie.name, 'movie', randomMovie.backdrop_path);
        }
...
async function saveToHistory(id, type, backdrop) {
    try {
        ...
        const savedObj = {
            id: data.id, title: data.title || data.name, poster_path: data.poster_path,
            backdrop_path: backdrop || data.backdrop_path,
            media_type: type, vote_average: data.vote_average || 0, 
            release_date: data.release_date || data.first_air_date || ''
        };

        let history = JSON.parse(localStorage.getItem('streamverse_history') || '[]');
        history = history.filter(m => m.id !== id); 
        history.unshift(savedObj); 
...
window.addEventListener('popstate', () => {
    ...
    if (!player.classList.contains('hidden')) {
        ...
        document.getElementById('playerBg').style.backgroundImage = 'none'; // Reset Tema
        ...
        try {
            ...
            suggestions.forEach(movie => {
                ...
                item.innerHTML = `
                    ...
                    <div class="flex-1 min-w-0"><h4 class="text-xs font-bold text-white truncate">${movie.title || movie.name}</h4><p class="text-[9px] text-white/50 mt-0.5">${year} • <span class="uppercase text-blue-400">${type}</span></p></div>
                `;
...
function renderCards(movies, container, append = false, isTV = false) {
    ...
    movies.forEach(movie => {
        ...
        // Passing backdrop_path untuk Adaptive Theme
        card.innerHTML = `
...
async function playMovie(id, title, type = 'movie', backdropPath = '') {
    ...
    saveToHistory(id, type, backdropPath); 
    ...
    // Set Adaptive Theme dari Backdrop
    if(backdropPath && backdropPath !== 'null') {
        bg.style.backgroundImage = `url('${BACK_PATH + backdropPath}')`;
    } else {
    ...
    try {
        const castRes = await fetch(`/api/movies?path=${type}/${id}/credits`);
                ...
                if(actor.profile_path) {
                    castHTML += `
                        <div class="flex flex-col items-center w-14 flex-shrink-0">
                            <img src="${IMG_PATH + actor.profile_path}" class="w-10 h-10 rounded-full object-cover shadow-lg border border-white/20">
